...
for i in range (1, 3000):
  ...
  liney.set_data( yp[3, :] , zp[:] )
  lineu.set_data( yp[3, :] , gp[:] )
  lineg.set_data( yp[3, :] , -yp[4, :] )
  #linez.set_data( yp[3, i] , -yp[4, i] )

  #pnty.set_data( x[3] , z_ref )
  #pntu.set_data( x[3] , z_terr )
  ...
  if i % 10 == 0: 
    ...
    txt = '{} ::  t = {:.9f}   V = {:.9f} km/h   gamma = {:.9f}   v_z = {:.9f} alt = {:.9f} teta={:.9f} ||| thr = {:.9f} elev = {:.9f} ||| e_v = {:.9f} e(q) = {:.9f} e(z) = {:.9f}'.format(problem_txt, t , V , gamma_ , v_z, alt, teta, thr, elev, e_v, e[2], e[5])
